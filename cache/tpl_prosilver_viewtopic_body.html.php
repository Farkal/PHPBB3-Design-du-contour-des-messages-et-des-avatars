	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post <?php if (($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?>bg1<?php } else { ?>bg2<?php } if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } ?>">
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dt>
				...
				<span style="background: url(<?php echo $_postrow_val['POSTER_CONTOUR_AVATAR']; ?>); width: 153px; height: 118px; text-align: center; padding: 20px 0px 0px; display: block; margin: 10px auto 0px;">
					<?php if ($_postrow_val['U_POST_AUTHOR']) {  ?><a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>"><?php echo $_postrow_val['POSTER_AVATAR']; ?></a><?php } else { echo $_postrow_val['POSTER_AVATAR']; } ?><br />
				</span>
